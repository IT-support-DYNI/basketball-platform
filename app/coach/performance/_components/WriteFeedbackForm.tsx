...
export default function WriteFeedbackForm({ players }: { players: PlayerOption[] }) {
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} className="rounded-full border border-line px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
        + Write Feedback
      </button>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-card border border-line bg-surface p-5 space-y-4">
      ...
      <textarea placeholder="Feedback message" value={message} onChange={(e) => setMessage(e.target.value)} required className="w-full rounded-control border border-line px-3 py-2.5 outline-none focus:border-court-500 focus:ring-2 focus:ring-court-500/20" />
